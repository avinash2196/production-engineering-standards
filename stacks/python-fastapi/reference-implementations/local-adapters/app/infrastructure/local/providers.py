...
Local adapters are available only as examples. Production adapters are loaded
only when an approved phase-specific Implementation Plan has added them.
"""
...
def _load_adapter(module_name: str, class_name: str) -> type[Any]:
    try:
        ...
        raise RuntimeError(
            f"{class_name} is not included in the reference implementation. "
...
def get_publisher() -> Any:
    ...
    if settings.messaging_adapter is MessagingAdapter.DB:
        _announce_local_adapter(
            "messaging", "db", "no broker partitions, consumer groups, or rebalancing"
        )
        return DatabaseOutboxMessagePublisher(
            SqlAlchemyOutboxStore(
                settings.database_url, table_name=settings.outbox_table_name
            )
        )
    if settings.messaging_adapter is MessagingAdapter.IN_MEMORY:
        _announce_local_adapter(
            "messaging", "inmemory", "process-local and lost on restart"
        )
        return InMemoryMessagePublisher()
    if settings.messaging_adapter is MessagingAdapter.PUBSUB:
        publisher_type = _load_adapter(
            "app.infrastructure.messaging.pubsub_publisher", "PubSubMessagePublisher"
        )
        return publisher_type(project_id=settings.gcp_project_id)
    publisher_type = _load_adapter(
        "app.infrastructure.messaging.kafka_publisher", "KafkaMessagePublisher"
    )
...
def get_cache() -> Any:
    ...
    if settings.cache_adapter is CacheAdapter.JSON_FILE:
        _announce_local_adapter(
            "cache", "jsonfile", "no distributed atomicity or multi-process consistency"
        )
        return JsonFileCacheProvider(settings.json_cache_path)
    if settings.cache_adapter is CacheAdapter.IN_MEMORY:
        _announce_local_adapter(
            "cache", "inmemory", "process-local and lost on restart"
        )
        return InMemoryCacheProvider()
    cache_type = _load_adapter(
        "app.infrastructure.cache.redis_cache", "RedisCacheProvider"
    )
...
def get_storage() -> Any:
    ...
    if settings.storage_adapter is StorageAdapter.LOCAL:
        _announce_local_adapter(
            "storage", "local", "no managed durability, IAM, or multi-instance behavior"
        )
    ...
    if settings.storage_adapter is StorageAdapter.GCS:
        storage_type = _load_adapter(
            "app.infrastructure.storage.gcs_storage", "GcsObjectStorageProvider"
        )
        return storage_type(project_id=settings.gcp_project_id, bucket=settings.gcs_bucket)
    storage_type = _load_adapter(
        "app.infrastructure.storage.s3_storage", "S3ObjectStorageProvider"
    )
    return storage_type(
...
def get_secrets() -> Any:
    ...
    if settings.secret_adapter is SecretAdapter.ENV:
        _announce_local_adapter(
            "secrets", "env", "no managed rotation, centralized policy, or audit trail"
        )
        return EnvSecretProvider()
    if settings.secret_adapter is SecretAdapter.SECRET_MANAGER:
        secret_type = _load_adapter(
            "app.infrastructure.secrets.secret_manager_provider", "SecretManagerProvider"
        )
        return secret_type(project_id=settings.gcp_project_id)
    secret_type = _load_adapter(
        "app.infrastructure.secrets.vault_provider", "VaultSecretProvider"
    )
