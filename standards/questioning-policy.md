...
2. Non-sensitive, well-scoped defaults such as using `MESSAGING_ADAPTER=db` (DB outbox) or `CACHE_ADAPTER=jsonfile` (JSON file cache) for local development when explicitly requested.
